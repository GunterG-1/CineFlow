...
function HomeUser() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const resp = await api.get('/cartelera/peliculas/cartelera');
        const list = Array.isArray(resp) ? resp : resp?.data ?? [];
        if (!mounted) return;
        setMovies(list || []);
        setFeaturedSlides((list || []).slice(0,3).map((m,i) => ({ id: i+1, movieId: m.id, imageSrc: m.bannerSrc || m.imageSrc, background: 'linear-gradient(115deg, #0f5bd7 0%, #38bdf8 20%)' })));
      } catch (err) {
  ...
  const handleSeleccionarHorario = ({ dia, nombreDia, fechaFuncionTexto, horario, precioSala, salaId, funcionId }) => {
    ...
    const peliculaConHorario = {
      ...
      // Agregar información de sala y precio
      salaId: salaId,
      precioSala: precioSala || peliculaSeleccionada.price,
      price: precioSala || peliculaSeleccionada.price, // También actualizar price
      idFuncion: funcionId, 
    };
