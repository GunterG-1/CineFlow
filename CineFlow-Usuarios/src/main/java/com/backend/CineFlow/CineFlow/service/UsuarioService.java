...
import com.backend.CineFlow.CineFlow.dto.UsuarioStatsResponse;
...
import com.backend.CineFlow.CineFlow.util.AdminRoleUtils;
...
public class UsuarioService {
    ...
    public UsuarioProfileResponse toProfile(Usuario usuario) {
        return new UsuarioProfileResponse(
            ...
            AdminRoleUtils.isAdminEmail(usuario.getCorreo())
        );
    }

        public UsuarioStatsResponse obtenerEstadisticas() {
        List<Usuario> usuarios = usuarioRepository.findAll();
        long usuariosAdmin = usuarios.stream()
            .filter(usuario -> AdminRoleUtils.isAdminEmail(usuario.getCorreo()))
            .count();
        long usuariosConMetodoPago = usuarios.stream()
            .filter(usuario -> usuario.getMetodoPago() != null && !usuario.getMetodoPago().isBlank())
            .count();

        return new UsuarioStatsResponse(usuarios.size(), usuariosAdmin, usuariosConMetodoPago);
        }
