...
public class ConfiiteroServicio {
    ...
    /**
     * Obtiene los alimentos (items) disponibles
     */
    public List<AlimentoDTO> obtenerAlimentos() {
        return alimentoRepositorio.findByActivoTrue()
            .stream()
            .map(alimento -> modelMapper.map(alimento, AlimentoDTO.class))
            .collect(Collectors.toList());
    }

    /**
     * Obtiene promociones persistidas en BD
     */
    public java.util.List<com.backend.CineFlow.CineFlow.dto.PromotionDTO> obtenerPromociones() {
        return promotionRepositorio.findByActivoTrue()
            .stream()
            .map(p -> new com.backend.CineFlow.CineFlow.dto.PromotionDTO(p.getId(), p.getTitle(), p.getDescription(), p.getDiscount(), p.getEmoji()))
