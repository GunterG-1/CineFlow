...
function HomeUser() {
  ...
  const { visibleMovies, featuredSlides } = useMovieCatalog();
  ...
  const handleClickSlide = (slide) => {
    const movie = visibleMovies.find((item) => item.id === slide.movieId);
  ...
  const handleSeleccionarHorario = ({ dia, nombreDia, fechaFuncionTexto, horario, salaId, precioSala, funcionId}) => {
    ...
    const peliculaConHorario = {
      ...
      salaId: salaId,
      precioSala: precioSala || peliculaSeleccionada.price,
      price: precioSala || peliculaSeleccionada.price,
      idFuncion: funcionId, 
    };
      ...
      <main className="home-user">
        ...
        <section className="home-user__grid">
          {visibleMovies.map((movie) => (
            <article key={movie.id} className="movie-card">
              ...
              <div className="movie-card__footer">
                <span className="movie-card__rating">{formatCLP(movie.price)}</span>
                <button
          ...
          {visibleMovies.length === 0 ? (
            <div className="home-user__empty-state">
              <h3>No hay peliculas visibles en este momento</h3>
              <p>Usa el panel admin para publicar titulos en la cartelera.</p>
            </div>
          ) : null}
        </section>
      </main>
