...
function ResumenPedido() {
  ...
  useEffect(() => {
    ...
    const cargarDisponibilidad = async () => {
      ...
      try {
        ...
        const respuesta = await api.get(`/entradas/disponibilidad?claveFuncion=${encodeURIComponent(claveFuncion)}`);
  ...
  const handleProcesarPago = async () => {
    ...
    try {
      // 1. BUSCAR EL ID DE LA FUNCIÓN CORRECTO
      // Usamos los datos de entradaPrincipal para encontrar la función que coincide en hora/minuto
      const funcionEncontrada = entradaPrincipal?.funciones?.find(f => {
        const [h, m] = f.hora.split(':').map(Number);
        return h === entradaPrincipal.horaNum && m === entradaPrincipal.minutoNum;
      });

      const idDeFuncion = funcionEncontrada?.id ?? null;
      ...
      // 2. Crear reserva
      // IMPORTANTE: El backend espera 'funcionId' según tu CreateReservationDto
      const reservaResp = await api.patch('/entradas/reservar', {
  idFuncion: idDeFuncion,
  asientosSeleccionados: selectedSeats,
});

      // 3. Extraer reservationId
      const reservationId = reservaResp?.data?.reservationId || reservaResp?.data?.id || reservaResp?.data?.data?.reservationId || reservaResp?.data?.data?.id || reservaResp?.reservationId || reservaResp?.id;
      if (!reservationId) throw new Error('No se recibió reservationId al crear la reserva');
