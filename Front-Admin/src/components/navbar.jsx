...
const Navbar = () => {
  ...
  const { isRegistered, getDisplayName, userProfile } = useAuth();
    ...
    <header className="navbar" aria-label="Navegacion principal">
      <div className="navbar__inner">
        ...
        <nav className={`navbar__menu ${menuOpen ? 'navbar__menu--open' : ''}`} id="navbar-menu" aria-label="Menu principal">
          ...
          {userProfile?.isAdmin ? (
            <>
              <Link key="Panel admin" to="/admin" className="navbar__link" onClick={() => setMenuOpen(false)}>
                Panel admin
              </Link>
              <Link key="Usuarios" to="/admin/usuarios" className="navbar__link" onClick={() => setMenuOpen(false)}>
                Usuarios
              </Link>
            </>
          ) : null}
